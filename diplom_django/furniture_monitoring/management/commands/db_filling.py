...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Import objects
        try:
            object_data = pd.read_csv("csv/object.csv")
        except Exception as e:
        ...
        for _, row in camera_data.iterrows():
            ...
            camera = (
                ...
                if camera_id and not pd.isna(camera_id)
                else None
            )
            ...
            if camera is None:
                self.stdout.write(
                    self.style.WARNING(
                        f"Skipping linecounter for missing camera_id {camera_id}"
                    )
                )
                continue
            ...
            if not LineCounter.objects.filter(
                camera=camera,
                start_x=start_x,
                start_y=start_y,
                end_x=end_x,
